...
# UI VERSION 3.0 - Premium Dark Trading Terminal
# All features preserved + Realistic charts
# PART 1 OF 3
...
# ============================================================
# Premium Dark Trading Terminal CSS v3.0
# ============================================================
...
:root {
    ...
    --border-accent:  rgba(0, 208, 156, 0.15);
    --border-hover:   rgba(0, 208, 156, 0.35);
    
    --accent-green:   #00D09C;
    --accent-green-dim: #00B887;
    --accent-red:     #F85149;
    --accent-red-dim: #D73A3A;
    ...
    --glow-red:       0 0 24px rgba(248, 81, 73, 0.12);
    --shadow-sm:      0 2px 8px rgba(0, 0, 0, 0.3);
    --shadow-md:      0 4px 16px rgba(0, 0, 0, 0.4);
    --shadow-lg:      0 8px 32px rgba(0, 0, 0, 0.5);
    --shadow-glow:    0 4px 24px rgba(0, 208, 156, 0.08);
...
.stApp {
    background: 
        radial-gradient(ellipse at top, rgba(0, 208, 156, 0.03) 0%, transparent 50%),
...
@media (max-width: 768px) {
    .block-container {
        padding: 40px 14px 12px 14px !important;
    }
    ...
    .stock-price { font-size: 20px !important; }
    [data-testid="stMetricValue"] { font-size: 16px !important; }
}

@media (max-width: 480px) {
    .block-container { padding: 40px 10px 10px 10px !important; }
...
def init_session_state():
    ...
    defaults = {
        ...
        "news_update_time":     0,
        "chart_timeframe":      "ALL",
        "chart_indicators":     {"sma5": True, "sma10": True, "volume": True},
    }
...
def update_prices():
    ...
    Data comes from YOUR database via generate_candles().
    """
    try:
        ...
        # Generate new candles - uses YOUR market.py + db.py
...
def get_portfolio_value():
    """Calculate total portfolio value - uses YOUR market.py"""
    try:
...
# ============================================================
# Place Order - Full Logic Preserved
# ============================================================
...
def check_pending_orders():
    ...
    Stop Loss SELL: Execute when price drops to/below trigger price.
    ...
    with tab2:
        ...
        if st.session_state.show_credentials and st.session_state.new_login_id:
            st.markdown(
                ...
                f'<p style="color:#F85149; font-size:12px; margin-top:10px;">'
                f'⚠️ Save these now! You cannot recover them later.'
                f'</p>'
                f'</div>',
                unsafe_allow_html=True
            )
                ...
                if not allowed:
                    ...
                    if new_pass:
                        ...
                        st.markdown(
                            ...
                            f'<p style="color:#F85149; font-size:12px; margin-top:10px;">⚠️ Save this now!</p>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                        st.code(
...
# ============================================================
# PART 2 OF 3
# Fragments (Realistic Chart) + Order Section + Dashboard
# ============================================================
...
def watchlist_fragment():
    """Watchlist - prices update every 5 sec from YOUR database"""
    update_prices()
...
# ============================================================
# FRAGMENT 2: REALISTIC TRADING CHART - Auto refresh every 8 sec
# Data source: YOUR MongoDB via get_candles()
# TradingView-style with SMA, Volume, Crosshair, Zoom, Pan
# ============================================================

@st.fragment(run_every=8)
def chart_fragment():
    """
    Realistic trading chart - all data from YOUR MongoDB.
    Features: Candlesticks, SMA5, SMA10, SMA20, Volume, Crosshair, Zoom.
    """
    ...
    # Get candles from YOUR database
    candles = get_candles(selected, limit=100)

    # Calculate day high/low/open from candles (from YOUR data)
    ...
    # Stock header with real stats from YOUR database
    st.markdown(
        f'<div class="stock-header">'
        f'<div>'
        f'<div class="stock-symbol">{selected}</div>'
        f'<div class="stock-exchange">NSE · Paper Trading · Live from DB</div>'
        f'</div>'
    ...
    # OHLC Stats bar (from YOUR database)
    stats_col1, stats_col2, stats_col3, stats_col4 = st.columns(4)
    with stats_col1:
        st.markdown(
            f'<div style="background:#0D1117;border:1px solid #1E2733;border-radius:8px;padding:8px 12px;">'
            f'<div style="font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:0.5px;">Open</div>'
    ...
    with stats_col2:
        st.markdown(
            f'<div style="background:#0D1117;border:1px solid #1E2733;border-radius:8px;padding:8px 12px;">'
            f'<div style="font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:0.5px;">High</div>'
    ...
    with stats_col3:
        st.markdown(
            f'<div style="background:#0D1117;border:1px solid #1E2733;border-radius:8px;padding:8px 12px;">'
            f'<div style="font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:0.5px;">Low</div>'
    ...
    with stats_col4:
        st.markdown(
            f'<div style="background:#0D1117;border:1px solid #1E2733;border-radius:8px;padding:8px 12px;">'
            f'<div style="font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:0.5px;">Prev Close</div>'
    ...
    # Timeframe selector (visual only - all data from DB)
    tf_col1, tf_col2, tf_col3, tf_col4, tf_col5, tf_col6 = st.columns([1,1,1,1,1,3])
    with tf_col1:
    ...
    if not candles or len(candles) < 3:
        st.markdown(
            '<div class="info-box">'
            '⏳ Generating live data from database... Please wait 30 seconds.'
            '</div>',
            unsafe_allow_html=True
        )
        return

    # Extract OHLC from YOUR database candles
    ...
    # Volume from YOUR candle data (calculated from OHLC range)
    volumes = [abs(highs[i] - lows[i]) * 1000 + abs(closes[i] - opens[i]) * 500 for i in range(len(closes))]

    # Determine visible range based on timeframe
    ...
    fig.add_trace(
        go.Candlestick(
            ...
            name=selected,
            increasing=dict(
                line=dict(color="#00D09C", width=1),
                fillcolor="#00D09C"
            ),
            decreasing=dict(
    ...
    # === SMA LINES from YOUR candle data ===
    if len(closes) >= 5:
    ...
    st.markdown(
        ...
        f'<div>🟡 SMA5 · 🔵 SMA10 · 🟣 SMA20 · Auto refresh 8s</div>'
        f'<div>🔄 Live from MongoDB</div>'
        f'</div>',
        unsafe_allow_html=True
    )
...
# ============================================================
# Order Section - Full Logic
# ============================================================

def order_section():
    """Order placement - Data from YOUR database"""
    prices = st.session_state.market_prices
    ...
    # Current position badge if owned
    if owned_qty > 0:
        ...
        st.markdown(
            ...
            f'text-transform:uppercase;color:#00D09C;margin-bottom:6px;">Current Position</div>'
            f'<div style="display:flex;justify-content:space-between;align-items:center;">'
            f'<div>'
            f'<div style="font-size:13px;color:#E6EDF3;font-weight:600;">{owned_qty} shares</div>'
            f'<div style="font-size:11px;color:#8B949E;margin-top:2px;">Avg: ₹{avg_price:.2f}</div>'
            f'</div>'
            f'<div style="text-align:right;">'
            f'<div style="font-size:13px;font-weight:600;color:{hp_color};">{hp_arrow} ₹{abs(holding_pnl):.2f}</div>'
            f'<div style="font-size:11px;color:#8B949E;margin-top:2px;">Unrealized P&L</div>'
            f'</div>'
            f'</div>'
            f'</div>',
            unsafe_allow_html=True
    ...
    with col_s:
        if owned_qty > 0:
            ...
            st.caption(f"💼 Holding: {owned_qty} | Avg: ₹{avg_price:.2f}")
        else:
            st.button(
                "▼ SELL",
                disabled=True,
    ...
    st.markdown(
        ...
        f'background:#0D1117; border:1px solid #1E2733;'
        f'border-radius:8px;">'
        f'<div style="font-size:10px;color:#8B949E;letter-spacing:0.8px;'
        f'text-transform:uppercase;margin-bottom:4px;">Available Balance</div>'
        f'<div style="font-size:18px;font-weight:700;'
        f'font-family:JetBrains Mono,monospace;color:#E6EDF3;">'
        f'₹{st.session_state.balance:,.2f}</div>'
        f'</div>',
        unsafe_allow_html=True
    )
...
# ============================================================
# Holdings Section - Full Logic
# ============================================================

def holdings_section():
    """Holdings - Data from YOUR portfolio + YOUR market prices"""
    ...
    if holdings_data:
        ...
        st.markdown(
            '<div class="info-box">No holdings yet. Buy a stock first!</div>',
            unsafe_allow_html=True
        )
...
def orderbook_section():
    ...
    if orders:
        ...
        st.markdown(
            '<div class="info-box">No orders yet. Place a trade!</div>',
            unsafe_allow_html=True
        )
...
def pending_orders_section():
    """Pending orders - from YOUR session state"""
    ...
    if pending:
        ...
        st.markdown(
            '<div class="info-box">No pending orders</div>',
            unsafe_allow_html=True
        )
...
def dashboard_page():
    ...
    st.markdown(
        ...
        '<div class="page-subtitle">Live Paper Trading Terminal · Data from MongoDB</div>'
        '</div>'
    ...
    with col1:
        st.markdown(
            ...
            f'<div class="metric-label">💰 Balance</div>'
            f'<div class="metric-value">₹{st.session_state.balance:,.0f}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    with col2:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">📊 Portfolio</div>'
    ...
    with col3:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">📈 Today P&L</div>'
            f'<div class="metric-value" style="color:{pnl_color};">'
            f'{pnl_arrow} ₹{abs(pnl):,.0f}</div>'
            f'<div class="metric-sub">Resets midnight IST</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    with col4:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">🏦 Net Worth</div>'
            f'<div class="metric-value">₹{net_worth:,.0f}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    with col5:
        ...
        st.markdown(
            ...
            f'<div class="metric-value">{total_trades}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
...
# ============================================================
# PART 3 OF 3
# All Other Pages + Sidebar + Router
# ============================================================
...
def history_page():
    ...
    if orders:
        ...
        st.markdown(
            '<div class="info-box">No trades yet. Start trading from Dashboard!</div>',
            unsafe_allow_html=True
        )
...
def funds_page():
    ...
    with col1:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">💵 Available Balance</div>'
            f'<div class="metric-value">₹{st.session_state.balance:,.2f}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    with col2:
        st.markdown(
            ...
            f'<div class="metric-value">₹{portfolio_value:,.2f}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    with col3:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">🏦 Net Worth</div>'
            f'<div class="metric-value">₹{net_worth:,.2f}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
...
def settings_page():
    ...
    st.markdown(
        '<div class="success-box">🌙 Dark Mode — Active & Locked</div>',
        unsafe_allow_html=True
    )
    st.divider()
...
def profile_page():
    """User profile page with bio edit and stats"""
    back_button()
    top_bar()
    ...
    with col2:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">Executed</div>'
            f'<div class="metric-value">{len(executed)}</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    with col3:
...
def leaderboard_page():
    ...
    if st.session_state.leaderboard_cache:
        ...
        st.markdown(
            '<div class="info-box">No users yet</div>',
            unsafe_allow_html=True
        )
...
def admin_panel():
    ...
    st.markdown(
        '<div class="error-box">'
        'After deletion, user cannot access their account (soft delete)'
        '</div>',
        unsafe_allow_html=True
    )
...
def analytics_page():
    ...
    if orders:
        ...
        with col2:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">Buy Orders</div>'
                f'<div class="metric-value" style="color:#00D09C;">{len(buy_exec)}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col3:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">Sell Orders</div>'
                f'<div class="metric-value" style="color:#F85149;">{len(sell_exec)}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col4:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">Brokerage Paid</div>'
                f'<div class="metric-value">₹{total_brokerage:,.2f}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        if pnl_data:
            ...
            st.markdown(
                '<div class="info-box">No sell trades yet for P&L chart</div>',
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            '<div class="info-box">No trade data for analytics yet</div>',
            unsafe_allow_html=True
        )
...
def options_page():
    """Options chain display - data based on YOUR spot prices"""
    back_button()
    top_bar()
    ...
    st.markdown(
        ...
        f'<div style="font-size:18px;font-weight:700;'
        f'font-family:JetBrains Mono,monospace;color:#E6EDF3;">₹{base:.2f}</div>'
...
# ============================================================
# AI Price Prediction Page - Uses YOUR market.py + db.py
# ============================================================

def predict_page():
    """
    AI price prediction using YOUR db engine + user prediction tool.
    All predictions from YOUR MongoDB via predict_next_move()
    """
    back_button()
    top_bar()
    ...
    st.markdown(
        ...
        f'<div style="font-size:18px;font-weight:700;'
        f'font-family:JetBrains Mono,monospace;color:#E6EDF3;">₹{curr_price:.2f}</div>'
        f'</div>',
        unsafe_allow_html=True
    )

    # Get prediction from YOUR market.py
    ...
    if prediction:
        ...
        with col1:
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">Predicted Price</div>'
                f'<div class="metric-value">₹{prediction["predicted_price"]:.2f}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col2:
            st.markdown(
                ...
                f'<div class="metric-value" style="color:{trend_color};">'
                f'{trend_arrow} {prediction["trend"]}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col3:
            st.markdown(
                ...
                f'<div class="metric-value">{prediction["strength"]:.2f}%</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col4:
            ...
            st.markdown(
                f'<div class="metric-card">'
                f'<div class="metric-label">RSI · {rsi_sig}</div>'
                f'<div class="metric-value" style="color:{rsi_color};">'
                f'{f"{rsi_val:.1f}" if rsi_val else "N/A"}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        ...
        with col1:
            st.markdown(
                f'<div class="card">'
                f'<div class="metric-label">SMA 5</div>'
                f'<div style="font-family:JetBrains Mono,monospace;font-size:16px;color:#E6EDF3;margin-top:4px;">'
                f'₹{prediction["sma_5"]:.2f}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col2:
            st.markdown(
                f'<div class="card">'
                f'<div class="metric-label">SMA 10</div>'
                f'<div style="font-family:JetBrains Mono,monospace;font-size:16px;color:#E6EDF3;margin-top:4px;">'
                f'₹{prediction["sma_10"]:.2f}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
        with col3:
            st.markdown(
                f'<div class="card">'
                f'<div class="metric-label">Support / Resistance</div>'
                f'<div style="font-family:JetBrains Mono,monospace;font-size:14px;color:#E6EDF3;margin-top:4px;">'
                f'₹{prediction.get("support", 0):.2f} / ₹{prediction.get("resistance", 0):.2f}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
    else:
        st.markdown(
            '<div class="info-box">⏳ Collecting market data from database... Please wait a moment and refresh.</div>',
            unsafe_allow_html=True
        )
    ...
    with col1:
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">Day 5 Predicted Price</div>'
    ...
    with col2:
        ...
        st.markdown(
            f'<div class="metric-card">'
            f'<div class="metric-label">AI Signal</div>'
            f'<div class="metric-value" style="color:{cp_color};">'
            f'{cp_arrow} {abs(change_perc):.2f}%</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    ...
    # Show saved predictions from YOUR database
    login_id   = st.session_state.current_user.get("login_id", "")
...
# ============================================================
# END OF PART 3 — FULL FILE COMPLETE
# ============================================================
